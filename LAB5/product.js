...
function displayProduct(product) {
  ...
  const imageElement = document.getElementById("product-image-detail");
...
function displayReviews(productId) {
  axios.get(`http://localhost:3100/reviews/${productId}`)
      .then(response => {
...
function addReview( content, rating, productId) {
    ...
    .then(() => {
        ...
        console.log('To see a new review, refresh the page')
    })
    .catch(error => {
...
function setupProduct() {
  ...
  const productImage = document.getElementById("product-image-detail");
